...
\begin{document}
	...
	\begin{abstract}
		In the last decade, computers and software we use have gained humanitarian characteristics. We talk with them, create chatbots that answer the user's question as a real assistant and many more. These features are invigorated by the Artificial Intelligence research, which has shown significant improvements over the past decade.
		
		With the emergence of the \gls{dl} systems, other sub-fields of AI such as \gls{nlp}  made leaps in a short amount of time as well. Today, the results of the studies of these fields are ubiquitous. \newline \par
		
		Development of such applications requires extensively large datasets for the training of \gls{dl} models. These applications are helping us in many ways. They enable us to solve our problems with less effort in a shorter amount of time, reduce the workloads on humans and much more. \newline \par
		...
		In this project, we aim to develop a Question Generation Model in the Turkish language.
		
		Furthermore, for an application to be developed in any language, training dataset must contain the examples from that language. Sourcing such data in languages such as English, German, French is not a challenging task. However, labelled data in the Turkish language is scarce; hence, the development of smart applications such as question bots, personal assistants in Turkish requires significantly more effort comparing to other languages.
		
		We believe that this dataset's development will significantly contribute to the development process of such applications.
	...
	\begin{abstractTR}
		Son on yılda kullandığımız bilgisayarlar ve yazılımlar insani özellikler kazandı. Onlarla konuşuyoruz, kullanıcının sorusuna gerçek bir asistan olarak cevap veren sohbetler ve çok daha fazlasını oluşturuyoruz. Tüm bu özellikler, son on yılda önemli gelişmeler gösteren Yapay Zeka araştırmasıyla canlandırıldı.
		
		\gls{dl} sistemlerinin ortaya çıkmasıyla birlikte, \gls{nlp} gibi diğer AI alt alanları da kısa sürede sıçramalar yaptı. Günümüzde bu alanlarla ilgili çalışmaların sonuçları her yerde bulunmaktadır. \newline \par
		
		Bu tür uygulamaların geliştirilmesi, \gls{dl} modellerinin eğitimi için oldukça büyük veri kümeleri gerektirir. Bu uygulamalar bize birçok yönden yardımcı oluyor. Sorunlarımızı daha az çabayla, daha kısa sürede çözmemizi, insanların üzerindeki iş yüklerini azaltmamızı sağlar, vb. \newline \par
		
		Bu tür uygulamaların geliştirilmesi, oldukça büyük veri kümeleri gerektirir. Bir \gls{dl} modelinin eğitilmesi için veri kümeleri çok önemlidir. Bu uygulamaların kapasitesi ve güvenilirliği, üzerinde eğitim aldıkları verilere büyük ölçüde bağlıdır. \newline \par
		
		Bu projede Türk Dilinde Soru Üretme Modeli geliştirmeyi hedefliyoruz.
		
		Ayrıca bir uygulamanın herhangi bir dilde geliştirilebilmesi için eğitim veri setinin o dilden örnekler de içermesi gerekir. Bu tür verileri İngilizce, Almanca, Fransızca vb. Dillerde temin etmek zor bir iş değildir. Ancak, Türkçe etiketli veriler az olduğundan, soru botları, kişisel asistanlar gibi akıllı uygulamaların Türkçe'de geliştirilmesi diğer dillere göre çok daha fazla çaba gerektirmektedir.
		
		Bu veri setinin geliştirilmesinin bu tür uygulamaların geliştirme sürecine önemli bir katkı sağlayacağına inanıyoruz.
		
		\anahtarkelimeler{Doğal Dil İşleme, Derin Öğrenme, Soru Üretme}
	\end{abstractTR}
	...
	Over time, developments in this area yield to a whole new class of possibilities and problems requiring brand new methodologies to be applied for reaching the solution. With the emergence of Machine Learning, computers became to be able to parse data, learn from the data and apply their findings from it in a way that they are told to.\\
	...
	This project has many social impacts, though not directly. With the solution we developed, strengthening the question-answering model carried out in connection with this project, we will aid the learners' process of knowledge acquisitions.
	...
	Also, the system must not give inaccurate information to the user, since they may depend on the system on a critical task whose fault cannot be tolerated.
	
	\section{Project Definition and Planning}
	Turkish Question Generation Model is a neural network-based model capable of generating questions from a given piece of text in the Turkish language. This project implements such model using \textit{Recurrent Neural Network (RNN)} and/or \textit{Convolutional Neural Network (CNN)}.
	...
	There is no GUI planned for the QG project; however, it will be presented as CLI and \textit{Python Module}\footnotemark \footnotetext{An importable Python file that contains the functions for incorporating Turkish Question Generation for further research.} servers. for public use if allowed.
	\\
	...
	In their paper, research engineers from Microsoft Research proposed extracting question from a given piece of text. \cite{duan2017question} While they used NLP methodologies and Neural Networks for their paper, they did not include the semantics of words in their paper.\\
	...
	However, the development of such a model requires an extensively large and labelled dataset consisting of paragraphs, related questions, and answers to those questions. Unfortunately, such dataset is not available in the Turkish language. There are examples of it, but they neither contain large amounts of the information nor find the data reliable. \newline \par
	
	For the reasons described above, we decided to compose our dataset. Our dataset consists of paragraphs from Turkish Wikipedia. Using the Personal entities from the Turkish Wikipedia, we can create the question patterns for each person's attributes. Later, these questions patterns are completed by substituting the person's name with the placeholders. At this step, we have our questions and the description paragraph of the person available to us.
	...
	Apart from the dataset, the project's primary goal is to create the \gls{qg} model. Given that this model \gls{qg} model will be developed based on the dataset, the first stage of this project has been dedicated to producing the dataset. 
	...
	\begin{itemize}
		...
		\item A \gls{qg} model.
	\end{itemize}
	This section will explain what kind of design process we have followed during the design, implementation, and testing of these two components separately.
	\subsection{Design}
	...
	During the initial stages, we also started to study the popular frameworks and libraries used for developing learning models. We started out with the \textit{PyTorch} library developed for Python Language. We used online courses, textbooks and official documentation to inspect the application available on GitHub while studying the PyTorch framework. \newline \par
	...
	Initially, we decided to move on similar topic simultaneously given that we were both novels about the topic. We sustained this approach until the later weeks of the project term. During the initial stages, we spend most of our time studying the concepts, enrolling into MOOC courses to understand the area and the typical applications. \newline \par
	...
	Then, Alp has joined to Erdal to fasten up the process after we realized that creating question patterns is a time-consuming process. Erdal \& Alp completed creating the question patterns together. Studies about BERT will continue as the dataset gets completed.\newline \par
	
	Other tasks such as reports, meeting notes distributed evenly between Erdal \& Alp.\newline \par
	
	Given that the project member-only consists of two people, we can't talk about organizational inefficiencies or miscommunication issues. 
