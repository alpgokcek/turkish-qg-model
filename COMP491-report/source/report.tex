...
\begin{document}
	...
	\begin{abstract}
		Over the last years, the software we use has gained humanitarian characteristics. These features are invigorated by the Artificial Intelligence research, which has shown significant improvements over the past decade. With the emergence of \gls{dl}, other sub-fields of AI such as \gls{nlp} made leaps in a short amount of time as well. Today, the results of the studies of these fields are ubiquitous. \newline \par
		
		Development of such applications requires extensively large datasets for the training of \gls{dl} models. Furthermore, capability and the reliability of these applications are highly dependent on the data they have been trained on. \newline \par
	...
	\gls{qg} models require extensively large training data. In this project, we develop the initial data sets for the development of the \gls{qg} model. At the later stages, dataset that we aggregated will be used to develop the final product which is the \gls{qg} Model itself.
	
	\subsubsection{Project Coverage}
	
	First part of the project, which is the main focus of this report, consist of building a data set as the training of \gls{qg} Model at the later stages. This data is based on the information retrieved from the Turkish Wikipedia. \newline \par
	
	Later stages of the project consists of sequential steps where the given input is passed over from one to other. These stages can be classified as the main content of the project. Initially, there will be a pre-processing stage for the data; this pre-process might be tweaking the string or simplifying the sentence. Then, BERT language model will be fine-tuned upon retrieving the input string.\\
