...
from data import BertDataset, Preprocess
...
def format_sentences(raw_content):
    formatted_questions = []
...
def generate_sentences(file_name):
    ...
    valid_set = BertDataset(bert_path / bert_model / 'requests' / file_name)
    ...
    encoder.to(device)
    f = open(f"../data/model/stage_one/decoder/model0epoch{str(epochs-1)}", 'rb')
    ...
    return format_sentences(str(sorted(loss_list, key=lambda x: x[0])[0:]))
