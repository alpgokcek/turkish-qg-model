...
def generate_sentences(filename):
    ...
    valid_set = BertDataset(filename)
    ...
    decoder = Decoder(bert_vocab_size, decoder_input_size,
                      bert_hidden_size, decoder_hidden_size, dropout, attention, device)
    model = Seq2Seq(decoder, device)

    encoder = BertModel.from_pretrained(
        "../data/model/stage_one/dbmdz/bert-base-turkish-cased/")
    encoder.to(device)
    f = open(
        "../data/model/stage_one/dbmdz/bert-base-turkish-cased/model0epoch9/pytorch_model.bin", 'rb')
    ...
    with torch.no_grad():
        ...
        for i, (input_, output_) in enumerate(valid_loader):
            ...
            prediction = model(bert_hs[0], output_data.to(
                device), 0)  # turn off teacher forcing
...
if __name__ == '__main__':
    generate_sentences(bert_path / bert_model / 'valid')
